...
public class MethodBlock extends Connection {

    public static String prefixb = "§7❒";
    static PreparedStatement stm = null;
    public static boolean contains(String p){

        try {
            stm = con.prepareStatement("SELECT * FROM `blocos` WHERE `player` = ?");
            stm.setString(1, p);
    ...
    public static void setPlayer(String p){
        try {
            stm = con.prepareStatement("INSERT INTO `blocos`(`player`, `quantia`) VALUES (?, ?)");
            stm.setString(1, p);
            stm.setDouble(2, 0);
            stm.executeUpdate();
    ...
    public static void set(String p, Double quantia){
        if (contains(p)){
            try {
                stm = con.prepareStatement("UPDATE `blocos` SET `quantia` = ? WHERE `player` = ?");
                stm.setDouble(1, quantia);
                stm.setString(2, p);
                stm.executeUpdate();
            } catch (SQLException e) {
                sc.sendMessage(prefix + " §cNão foi possível setar o blocos do usuário.");
            }
        } else {
    ...
    /*public static void delete(String p){
        if (contains(p)){
            try {
                stm = con.prepareStatement("DELETE FROM `blocos` WHERE `Player` = ?");
    ...
    /*public static List<String> getTops(){
        ...
        try {
            stm = con.prepareStatement("SELECT * FROM `blocos` ORDER BY `quantia` DESC LIMIT 10");
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                    tops.add("§8➟§7 " + rs.getString("player") + " §8▶ " + prefixb +
                            decimal.formatSimbolo(rs.getDouble("quantia")));
            }
        } catch (SQLException e) {
            sc.sendMessage(prefix + " §cNão foi possível gerar a lista de top blocos.");
        }
        return tops;
    }*/
    ...
    public static List<String> getTop(){
        ...
        try {
            stm = con.prepareStatement("SELECT * FROM `blocos` ORDER BY `quantia` DESC LIMIT 5");
            ResultSet rs = stm.executeQuery();
            int i = 0;
            while (rs.next()) {
                i++;
                tops.add(rs.getString("player"));
                tops.add(prefixb + Decimal.formatSimbolo(rs.getDouble("quantia")));
            }
        } catch (SQLException e) {
            sc.sendMessage(prefix + " §cNão foi possível gerar a lista de top blocos.");
        }
