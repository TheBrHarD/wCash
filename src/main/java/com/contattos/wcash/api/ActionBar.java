...
public class ActionBar {
	...
	public static void sendAction(Player player, String message) {
		if (nmsVersion == null) {
			nmsVersion = (nmsVersion = Bukkit.getServer().getClass().getPackage().getName())
					.substring(nmsVersion.lastIndexOf(".") + 1);
		}
		try {
			Class<?> c1 = Class.forName("org.bukkit.craftbukkit." + nmsVersion + ".entity.CraftPlayer");
			Object p = c1.cast(player);
			Object ppoc;
			Class<?> c2, c3, c4 = Class.forName("net.minecraft.server." + nmsVersion + ".PacketPlayOutChat"),
